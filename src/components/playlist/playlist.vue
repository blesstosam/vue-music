                ...
                <Scroll :refreshDelay="refreshDelay" ref="listContent" class="list-content" :data="sequenceList">
                    <transition-group tag="ul" name="list">
                        ...
                        @click="selectItem(item,index)">
                            ...
                            <span class="delete" @click.stop="deleteOneSong(item)"><i class="icon-delete"></i></span>
                        </li>
                    </transition-group>
                </Scroll>
...
export default {
    ...
    computed: {
        modeText() {
            return this.mode === playMode.sequence ? '随机播放' : this.mode === playMode.loop ? "单曲循环" : '顺序播放';
        }
    },
    components: {Scroll, Confirm, AddSong},
    watch: {
        currentSong(n,o) {
            if (!this.showFlag || n.id === o.id) {
                return;
            }
            setTimeout(() => {
                this.scrollToCurrent(n);
            }, 20);
        }
    },
    methods: {
